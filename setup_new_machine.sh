...
echo "  ram@maxiomwealth.com | +91-9550290118"
echo ""
echo "──────────────────────────────────────────────────────────────────"
...
if [ ! -f "$CREDS_FILE" ]; then
    ...
    echo ""
    python3 "$ARUNI_DIR/.aruni/encrypt_creds.py" "$ARUNI_PASSWORD"
    if [ ! -f "$CREDS_FILE" ]; then
        exit 1
    fi
fi
...
echo "  python3 .aruni/daily_email.py     Send today's review email now"
echo ""
